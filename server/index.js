...
function emptyState() {
  return {
    ...
    flights: {
      ...
      tripYear: null, daysUntilTrip: null, waitingFor: null, note: null,
    },
  }
}
...
function migrate(s) {
  ...
  // Discard fares belonging to a different trip period.
  //
  // Prices are only meaningful for the window they were quoted in. Once the
  // trip year moves forward, the stored legs are last season's research: leaving
  // them in place makes the board present stale dates (and a stale "cheapest
  // combo") as if they were current, and nothing ever overwrites them while the
  // new trip is still outside the booking horizon.
  const TRIP_YEAR = Number(process.env.TRIP_YEAR) || 2027
  const keep = (s.flights.legs || []).filter(l => {
    const y = String(l.date || '').slice(0, 4)
    return y === String(TRIP_YEAR)
  })
  if (keep.length !== (s.flights.legs || []).length) {
    const dropped = (s.flights.legs || []).length - keep.length
    s.flights = {
      ...base.flights,
      tripYear: TRIP_YEAR,
      error: null,
      note: `se descartaron ${dropped} tarifas de un período anterior del viaje`,
    }
  }
  if (s.flights.tripYear === undefined) s.flights.tripYear = TRIP_YEAR
...
async function loadState() {
  try {
    ...
    const before = JSON.stringify(raw.flights ?? null)
    state = migrate(raw)
    // Migration can discard stale data (e.g. fares from an earlier trip period).
    // If it does, write it back — otherwise every boot re-reads the same stale
    // fares and the board keeps showing last season's prices.
    if (JSON.stringify(state.flights ?? null) !== before) {
      await fsp.mkdir(path.dirname(STORE), { recursive: true })
      await fsp.writeFile(STORE, JSON.stringify(state, null, 2))
      if (state.flights?.note) console.log(`[migrate] ${state.flights.note}`)
    }
  } catch {
    state = emptyState()
  }
...
async function refreshRoutes(dayByDay = false) {
  const { TRIP, bookingDateGrid, daysUntil } = await import(path.join(ROOT, 'src/data/trip.js'))
  const outDates = bookingDateGrid(dayByDay ? 1 : 3)
...
async function autoRefreshFlights(reason) {
  ...
  try {
    ...
    if (!routes.outbound.dates.length) {
      // Still outside the booking horizon. Report the exact day fares start so
      // the state carries a real answer instead of a bare "nothing yet".
      const { bookingTimeline } = await import(path.join(ROOT, 'src/data/trip.js'))
      const t = bookingTimeline()
      await mutate(s => {
        s.flights = {
          ...s.flights,
          ...
          waitingFor: {
            firstFaresOn: t.firstFaresOn,
            daysToFirstFares: t.daysToFirstFares,
            horizon: t.horizon,
          },
          error: null,
        }
      })
      console.log(`[flights] fuera del horizonte de reserva (${
        routes.daysUntilTrip} días para el viaje). Primeros precios el ${
        t.firstFaresOn} (en ${t.daysToFirstFares} días).`)
      return
    }
...
seedIfEmpty().then(() => {
  ...
  // Refresh fares shortly after boot, then daily. Daily (not 12h) is deliberate:
  // the whole point is to catch the moment a new date enters the booking horizon.
  // While the trip is still outside it, this is a cheap no-op that just records
  // the date fares will start. Delayed a little so boot and the first health
  // checks are not blocked by a network call.
  const hours = Number(process.env.FLIGHT_REFRESH_HOURS) || 24
